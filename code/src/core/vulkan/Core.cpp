...
void vk_core_init(core_t* vk_core, const window_t& window) 
{
	...
	pipeline_config_setup(&vk_core->pipeline_cfg, vk_core->swapchain_cfg, vk_core->shader_modules);
	pipeline_create(&vk_core->pipeline, vk_core->device, vk_core->pipeline_cfg, vk_core->pipeline_layout, vk_core->render_pass);
	framebuffers_create(&vk_core->swapchain_cfg, vk_core->device, vk_core->render_pass);
...
void vk_core_destroy(core_t* vk_core) 
{
	...
	framebuffers_destroy(vk_core->swapchain_cfg, vk_core->device);
	...
	img_views_destroy(vk_core->swapchain_cfg, vk_core->device);
