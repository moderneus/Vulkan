...
void core_init(core *c, const window &win) 
{
	...
	pipeline_layout_create(&c->lyt, c->dev);
	pipeline_create(&c->pl, c->dev, c->lyt, c->rp, c->swp_st, c->shdrs);
	compute_pipeline_layout_create(&c->comp_lyt, c->dev, c->set_lyt);
	compute_pipeline_create(&c->comp_pl, c->dev, c->comp_lyt, c->shdrs[2]);
	...
	command_buffers_create(&c->comp_cmds, c->dev, c->cmd_pool);
	uniform_buffer_create(&c->uniform_bufs, c->dev, c->gpu);
	shader_storage_buffer_create(&c->ssbos, c->dev, c->gpu, c->q, c->cmd_pool);
	descriptor_pool_create(&c->set_pool, c->dev);
	descriptor_sets_create(&c->sets, c->dev, c->set_pool, c->set_lyt, c->uniform_bufs, c->ssbos);
	semaphores_create(&c->img_avail_sems, c->dev);
	semaphores_create(&c->rnd_done_sems, c->dev);
	semaphores_create(&c->comp_done_sems, c->dev);
	fences_create(&c->frm_fences, c->dev);
	fences_create(&c->comp_frm_fences, c->dev);
...
void core_destroy(core *c) 
{
	...
	fences_destroy(c->comp_frm_fences, c->dev);
	fences_destroy(c->frm_fences, c->dev);
	semaphores_destroy(c->comp_done_sems, c->dev);
