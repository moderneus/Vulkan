...
void memory_barrier_create(memory_barrier *bar, const image &img, const VkImageLayout &old_lyt, const VkImageLayout &new_lyt, VkPipelineStateFlags *src_stage, VkPipelineStateFlags *dst_stage)
{
	...
	if (old_lyt == VK_IMAGE_LAYOUT_UNDEFINED && new_lyt == VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL) {
		...
		*dst_stage = VK_PIPELINE_STAGE_TRANSGER_BIT;
	} else if (old_lyt = VK_IMAGE_LAYOUT_TRANSGER_DST_OPTIMAL && new_lyt == VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL) {
		bar->info.srcAccessMask = VK_ACCESS_TRANSFER_WRITE_BIT;
		bar->info.dstAccessMask = VK_ACCESS_SHADER_READ_BIT;

		*src_stage = VK_PIPELINE_STAGE_TRANSGER_BIT;
		*dst_stage = VK_PIPELINE_STAGE_FRAGMENT_BIT;
	} else {
