...
std::vector<const char*> inst_get_req_exts() 
{
	...
	std::vector<const char*> exts_vec(exts, exts + exts_cnt);
...
void instance_create(instance_t *inst) 
{
	...
	VkApplicationInfo app_info = inst_create_app_info();
	VkDebugUtilsMessengerCreateInfoEXT msgr_info = dbg_msgr_create_info();
	std::vector<const char*> exts = inst_get_req_exts();
	VkInstanceCreateInfo inst_info = inst_create_info(&app_info, &msgr_info, exts);

	if (vkCreateInstance(&inst_info, nullptr, &inst->handle) != VK_SUCCESS) {
